...
def parseRecieved (text):
    url = text.split()
    if url[1] == "/time" :
        return page % time.time()
    else :
...
while True: # just keep serving page to any client that connects
    ...
    recieved = client.recv(size) # HTTP request - not too big!  Just ignore contents
    send = parseRecieved (recieved)
    client.send(send) # HTTP response - same for any request
    client.close()
